...
| 항목                                                             | 확정값                                                                                                                                                                                                                                                   |
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 식별자(`game:action`, `game:sync`, `game:prompt_response`) | `gameId`만 허용. `roomId`는 게임 소켓 payload에서 미사용                                                                                                                                                                                             |
| ActionType(건설 포함)                                            | `ROLL_DICE`, `BUY_PROPERTY`, `SELL_PROPERTY`, `END_TURN` 고정. 건설은 `BUY_PROPERTY` 재사용(자동 단계 상승). `BUILD_PROPERTY` 미지원                                                                                                         |
| `game:prompt` 식별자                                           | `promptId`가 정식 필드. `id`는 레거시 별칭으로만 허용                                                                                                                                                                                                |
| `game:prompt` 타임아웃 필드                                    | 정식은 `timeoutSec`. `payload.timeoutMs`는 레거시 별칭                                                                                                                                                                                               |
| `game:prompt`의 `choices`                                    | 선택형 프롬프트(`BUY_OR_SKIP` 등)에서는 필수, 알림형 프롬프트에서는 선택                                                                                                                                                                               |
| `game:prompt_response` 이후 `game:ack`                       | 프롬프트 응답에 대한 `ack`에는 `promptId`를 포함해야 함                                                                                                                                                                                              |
| 프롬프트 실패 코드 규칙                                          | `PROMPT_NOT_FOUND`, `PROMPT_EXPIRED`, `INVALID_PROMPT_CHOICE`, `NOT_PROMPT_OWNER`, `INVALID_PHASE` 사용                                                                                                                                        |
| snapshot 최종 스키마                                             | `snapshot.turn`, `snapshot.currentPlayerId`, `snapshot.phase`, `snapshot.players[]`, `snapshot.tiles[]` 고정. 플레이어 위치=`currentTileId`, 소유권=`ownedTiles[]` + `tiles[].ownerId`, 상태=`playerState`                             |
| PlayerState 최종값                                               | `NORMAL`, `LOCKED`, `BANKRUPT`                                                                                                                                                                                                                     |
| phase 최종값                                                     | `WAIT_ROLL`, `MOVING`, `RESOLVING`, `WAIT_PROMPT`, `TURN_END`, `GAME_OVER`                                                                                                                                                                   |
| ServerEventType 최종값                                           | `DICE_ROLLED`, `PLAYER_MOVED`, `LANDED`, `PAID_TOLL`, `BOUGHT_PROPERTY`, `SOLD_PROPERTY`, `TURN_ENDED`, `PLAYER_STATE_CHANGED`, `CHANCE_RESOLVED`, `SYNCED` (`TURN_ENDED` 포함)                                                    |
| revision / sync 정책                                             | `knownRevision`과 서버 `revision` 동일 시 `SYNCED` 이벤트만 포함된 빈 patch 가능. 누락분 기준 단위는 **revision count**이며 `serverRevision - knownRevision > 200`이면 snapshot 강제 반환(또는 서버 보관분 없음/클라 revision 미래값 포함) |
| out-of-order 패킷 처리                                           | 클라는 `revision <= localRevision` 패킷을 무시                                                                                                                                                                                                         |
| money 단위                                                       | canonical unit은**만원 단위 정수**. BE 전 구간 동일 적용                                                                                                                                                                                           |
| `PAID_TOLL` 이후 인수 플로우                                   | v1 미지원(인수 prompt 미발행). 랜드마크 제외/건물 승계 규칙은 차기 버전에서 별도 추가                                                                                                                                                                    |
| 임시 호환 종료 계획(요약)                                        | `roomId fallback`은 즉시 종료. 프롬프트 레거시 별칭(`id`, `timeoutMs`)만 한시 지원                                                                                                                                                                 |
